...
function Hi = calculate_Hi_cartesian_my_version( Xv_km1_k, yi, camera, i, features_info )
...
function a = dhd_dhu( camera, zi_d )
    ...
    inv_a = jacob_undistor_fm_my_version( camera, zi_d );
    a=inv(inv_a);
...
%     kv = 1/camera.dy;
    ...
    hrly = hrl(2);
    hrlz = hrl(3);
    a = [f/(hrlz)       0           -hrlx*f/(hrlz^2);
        0               f/(hrlz)    -hrly*f/(hrlz^2)];
