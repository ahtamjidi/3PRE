...
static void gtku_gl_image_area_finalize (GObject *obj);

G_DEFINE_TYPE (GtkuGLImageArea, gtku_gl_image_area, GTKU_TYPE_GL_DRAWING_AREA);
...
static void
gtku_gl_image_area_init (GtkuGLImageArea *self)
{
    self->target = 0;
    self->int_format = 0;
    self->format = 0;
...
static void
gtku_gl_image_area_class_init (GtkuGLImageAreaClass *klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
    // add a class-specific destructor
    gobject_class->finalize = gtku_gl_image_area_finalize;
}

// destructor (more or less)
static void
gtku_gl_image_area_finalize (GObject *obj)
{
    GtkuGLImageArea *self = GTKU_GL_IMAGE_AREA (obj);
    if (self->texname) {
    ...
    G_OBJECT_CLASS (gtku_gl_image_area_parent_class)->finalize (obj);
}

GtkWidget *
gtku_gl_image_area_new ()
{
    return GTK_WIDGET (g_object_new (GTKU_TYPE_GL_IMAGE_AREA, NULL));
}

static gboolean 
on_gl_expose (GtkWidget * widget, GdkEventExpose * event, void* user_data)
{
    GtkuGLImageArea *self = GTKU_GL_IMAGE_AREA (user_data);

    // activate the opengl context
    gtku_gl_drawing_area_set_context (GTKU_GL_DRAWING_AREA (self));
...
{
    GtkuGLImageArea *self = GTKU_GL_IMAGE_AREA (user_data);
    gtku_gl_drawing_area_swap_buffers (GTKU_GL_DRAWING_AREA (self));
    return FALSE;
}
...
int
gtku_gl_image_area_set_image_format (GtkuGLImageArea *self,
        int width, int height, GLenum format)
{
    if (format != GL_LUMINANCE && 
        ...
        format != GL_BGRA)
    {
        errl ("Error: GtkuGLImageArea does not support GL format %s\n",
...
int
gtku_gl_image_area_upload_image (GtkuGLImageArea * self,
