...
typedef struct _app_t 
{
    ...
    GtkuGLImageArea *gl_area;
    GtkuParamWidget *param_widget;
} app_t;
...
on_gl_area_expose (GtkWidget * widget, GdkEventExpose * event, void* user_data)
{
    ...
    if (gtku_param_widget_get_bool (self->param_widget, "Draw X")) {
        glColor3f (0, 1, 0);
        glBegin (GL_LINES);
        glVertex2f (0, 0);
...
app_create (const app_params_t *params)
{
    ...
    gtku_gl_image_area_set_image_format (self->gl_area,
            self->img_width, self->img_height, GL_RGB);
    gtku_gl_image_area_upload_image (self->gl_area, self->img, 
            self->img_stride);
...
setup_gui (app_t *self)
{
    ...
    self->gl_area = GTKU_GL_IMAGE_AREA (gtku_gl_image_area_new (FALSE));
    ...
    self->param_widget = GTKU_PARAM_WIDGET (gtku_param_widget_new ());
    gtk_box_pack_start (GTK_BOX (hbox), GTK_WIDGET (self->param_widget), 
            FALSE, TRUE, 0);
    gtku_param_widget_add_booleans (self->param_widget, 0, 
            "Draw X", 1, NULL);
...
int main (int argc, char **argv)
{
    ...
    opt_parser = g_option_context_new (" - GtkuGLImageArea tester");
