...
struct _GLUtilSPlot2d {
    ...
    int show_ylim;
    GLUtilSPlot2dLegendLocation show_legend;
...
static inline int _set_color (double rgba[4], double r, double g, double b,
    ...
    if (r < 0 || g < 0 || b < 0 || r > 1 || g > 1 || b > 1 || a < 0 || a > 1) {
        g_warning ("GLUtilSPlot2d refusing to set invalid color "
                "%f, %f, %f, %f\n",
                r, g, b, a);
        return -1;
    }
...
glutil_splot2d_new ()
{
    GLUtilSPlot2d *self = g_slice_new0 (GLUtilSPlot2d);
    self->x_min = 0;
    self->x_max = 1;
    self->bg_rgba[0] = 0;
    self->bg_rgba[1] = 0;
    ...
    self->show_legend = GLUTIL_SPLOT2D_HIDDEN;
...
void
glutil_splot2d_free (GLUtilSPlot2d *self)
{
    memset (self, 0, sizeof (self));
    g_slice_free (GLUtilSPlot2d, self);
}

int 
glutil_splot2d_set_title (GLUtilSPlot2d *self, const char *title)
{
...
void
glutil_splot2d_set_show_title (GLUtilSPlot2d *self, int val)
{
    self->show_title = val;
}

int 
glutil_splot2d_add_plot (GLUtilSPlot2d *self, const char *name,
        int max_points)
{
    if (max_points <= 0) {
        g_warning ("GLUtilSPlot2d: bad max_points (%d), forcing to 1000\n",
                max_points);
        max_points = 1000;
    }
    ...
    if (existing) {
        g_warning ("GLUtilSPlot2d: replacing existing plot %s\n", name);
    }
...
int 
glutil_splot2d_remove_plot (GLUtilSPlot2d *self, const char *name)
{
...
int
glutil_splot2d_add_point (GLUtilSPlot2d *self, const char *name,
        double x, double y)
{
...
int 
glutil_splot2d_set_color (GLUtilSPlot2d *self, const char *name, 
...
int 
glutil_splot2d_set_show_legend (GLUtilSPlot2d *self,
        GLUtilSPlot2dLegendLocation where)
{ self->show_legend = where; return 0; }

int
glutil_splot2d_set_xlim (GLUtilSPlot2d *self, double xmin, double xmax)
{
    if (xmin >= xmax) {
        g_warning ("GLUtilSPlot2d refusing to set xmin, max to %f, %f\n",
                xmin, xmax);
        return -1;
    }
...
int
glutil_splot2d_set_ylim (GLUtilSPlot2d *self, double ymin, double ymax)
{
    if (ymin >= ymax) {
        g_warning ("GLUtilSPlot2d refusing to set ymin, max to %f, %f\n",
                ymin, ymax);
        return -1;
    }
...
_plot2d_render_window (void *key, void *value, void *user_data)
{
    ...
    GLUtilSPlot2d *sp = (GLUtilSPlot2d*) user_data;
...
static void
_draw_text (GLUtilSPlot2d *self, const char *text)
{
...
static void
draw_title (GLUtilSPlot2d *self, int x, int y, int width, int height)
{
...
static void
draw_axis_labels (GLUtilSPlot2d *self, int x, int y, int width, int height)
{
...
static void
draw_legend (GLUtilSPlot2d *self, int x, int y, int width, int height)
{
    if (self->show_legend == GLUTIL_SPLOT2D_HIDDEN) return;
    ...
    switch (self->show_legend) {
        case GLUTIL_SPLOT2D_TOP_LEFT:
            legend_x = x;
            legend_y = y;
            break;
        case GLUTIL_SPLOT2D_TOP_RIGHT:
            legend_x = x + width - legend_width;
            legend_y = y;
            break;
        case GLUTIL_SPLOT2D_BOTTOM_LEFT:
            legend_x = x;
            legend_y = y + height - legend_height;
            break;
        case GLUTIL_SPLOT2D_BOTTOM_RIGHT:
            legend_x = x + width - legend_y;
            legend_y = y + height - legend_height;
            break;
        default:
            g_warning ("GLUtilSPlot2d: invalid legend location %d\n", 
                    self->show_legend);
            goto done;
    }
...
void 
glutil_splot2d_gl_render_at_window_pos (GLUtilSPlot2d *self,
