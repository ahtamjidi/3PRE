...
TODO: port this to use g_slice instead of malloc
...
static int count_optimal_vertices(gpc_vertex_list c)
{
  ...
  for (i= 0; i < c.npoints; i++)
    ...
    if (OPTIMAL(c.points, i, c.npoints))
      result++;
  return result;
}
...
static edge_node *build_lmt(lmt_node **lmt, sb_tree **sbtree,
                            int *sbt_entries, const gpc_polygon *p, int type,
                            gpc_op op, const uint8_t * contributes)
{
  ...
  for (c= 0; c < p->num_contours; c++)
  {
    if (contributes[c])
    {
      ...
      for (i= 0; i < p->contour[c].npoints; i++)
        if (OPTIMAL(p->contour[c].points, i, p->contour[c].npoints))
        {
          edge_table[npoints].vertex.x= p->contour[c].points[i].x;
          edge_table[npoints].vertex.y= p->contour[c].points[i].y;

          /* Record vertex in the scanbeam table */
          add_to_sbtree(sbt_entries, sbtree,
                        edge_table[npoints].vertex.y);
...
static bbox *create_contour_bboxes(const gpc_polygon *p)
{
  bbox *box;
  int   c, v;
  ...
  for (c= 0; c < p->num_contours; c++)
  {
    ...
    {
      /* Adjust bounding box */
      if (p->contour[c].points[v].x < box[c].xmin)
        box[c].xmin= p->contour[c].points[v].x;
      if (p->contour[c].points[v].y < box[c].ymin)
        box[c].ymin= p->contour[c].points[v].y;
      if (p->contour[c].points[v].x > box[c].xmax)
        box[c].xmax= p->contour[c].points[v].x;
      if (p->contour[c].points[v].y > box[c].ymax)
          box[c].ymax= p->contour[c].points[v].y;
    }
  }
  return box;  
}
...
static void minimax_test(const gpc_polygon *subj, const gpc_polygon *clip,
        gpc_op op, uint8_t * subj_contributes, uint8_t * clip_contributes)
{
  bbox *s_bbox, *c_bbox;
  ...
  for (c= 0; c < clip->num_contours; c++)
  {
    ...
    if (!overlap)
      clip_contributes[c] = 0;
    else
      clip_contributes[c] = 1;
  }  
  ...
  {  
    ...
    for (s= 0; s < subj->num_contours; s++)
    {
      ...
      if (!overlap)
        subj_contributes[s] = 0;
      else
        subj_contributes[s] = 1;
    }  
  }
...
void gpc_write_polygon(FILE *fp, int write_hole_flags, gpc_polygon *p)
{
  ...
  for (c= 0; c < p->num_contours; c++)
  {
    ...
    for (v= 0; v < p->contour[c].npoints; v++)
      fprintf(fp, "% .*lf % .*lf\n",
              DBL_DIG, p->contour[c].points[v].x,
              DBL_DIG, p->contour[c].points[v].y);
  }
}
...
void gpc_add_contour(gpc_polygon *p, gpc_vertex_list *new_contour, int hole)
{
  ...
  extended_contour[c].npoints= new_contour->npoints;
  MALLOC(extended_contour[c].points, new_contour->npoints
         * sizeof(gpc_vertex), "contour addition", gpc_vertex);
  for (v= 0; v < new_contour->npoints; v++)
    extended_contour[c].points[v]= new_contour->points[v];
...
void gpc_polygon_clip(gpc_op op, const gpc_polygon *subj,
        const gpc_polygon *clip, gpc_polygon *result)
{
  sb_tree       *sbtree= NULL;
  ...
  uint8_t subj_contributes[subj->num_contours];
  uint8_t clip_contributes[clip->num_contours];
  memset (subj_contributes, 1, subj->num_contours);
  memset (clip_contributes, 1, clip->num_contours);
  /* Identify potentialy contributing contours */
  if (((op == GPC_INT) || (op == GPC_DIFF))
   && (subj->num_contours > 0) && (clip->num_contours > 0))
    minimax_test(subj, clip, op, subj_contributes, clip_contributes);
  ...
  if (result->num_contours > 0)
  {
    ...
    for (poly= out_poly; poly; poly= npoly)
    {
      ...
      if (poly->active)
      {
        ...
        MALLOC(result->contour[c].points,
          result->contour[c].npoints * sizeof(gpc_vertex),
        ...
        {
          nv= vtx->next;
          result->contour[c].points[v].x= vtx->x;
          result->contour[c].points[v].y= vtx->y;
          FREE(vtx);
          v--;
        }
        c++;
