...
%% DEBUG uncomment the second part and comment the first part 
%%% With this code every time we initialize all available fetures into the
%%% map
...
count_type=0;
for i=1:length(features_info)
%     features_info(i).current_step = step;
...
if strcmp(myCONFIG.FLAGS.VERBOSE,'yes')
    count_type
    percent_cart=count_type/length(features_info);
end
