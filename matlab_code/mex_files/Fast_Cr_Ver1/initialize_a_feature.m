...
for i=1:max_initialization_attempts
    ...
    if strcmp(myCONFIG.FLAGS.FEATURE_EXTRACTOR,'FAST')
        cs = fast_corner_detect_9_mex(imTemp2,... % the image,
            10);
    ...
    if strcmp(myCONFIG.FLAGS.FEATURE_EXTRACTOR,'SIFT')
        ...
        [Frames,Descriptor]=sift_vedal(imTemp2);
        %%%%
        %%% LOWE's implementation
        %         [II,Descriptor,Frames] = sift(imTemp2);
        %         Frames = [Frames(:,2)';Frames(:,1)';Frames(:,3:4)'];
        %%%
        all_uv = Frames(1:2,:); %% all_uv should be a matrix whose columns are feature points (SIFT gives frames in rows)
        %%% also lowe sift gives (row column) = (y,x) coordinate but in
        %%% image coordinate we need (x,y) = (column row)
    end
    ...
    %     if FAST corner detector
    %     all_uv = c';
    %     if sift corner detector
    % all_uv = c;
    %     cd ..
    ...
    if(~isempty(uv))
        ...
        %%% TAMADD
        [initial_rho,Feature3d_in_code_coordinate]=inittialize_depth_my_version(uv, step);
        %%%
        ...
        if strcmp(myCONFIG.FLAGS.FEATURE_EXTRACTOR,'SIFT')
            features_info = add_feature_to_info_vector_my_version_sift( uv, im_k, X_RES, features_info, step, newFeature,Feature3d_in_code_coordinate,Descriptor(:,1) );
        end
