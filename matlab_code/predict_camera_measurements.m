...
%% DEBUG 
        hi = hi_inverse_depth( yi, t_wc, r_wc, cam );
        if ~isreal(hi)
            disp('hi is imaginary')
        end
%%
        %%  For Debug
%         try
%         if norm(hi-features_info(i).init_measurement)>0.001
%             disp('Dadam Vay')
%         end
%         catch
%             disp('norm(hi-features_info(i).init_measurement failed')
%         end
        %%
        if (~isempty(hi))
            features_info(i).h = hi';
