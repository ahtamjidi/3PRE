...
for i = 1:length(features_info)
    ...
    %     %%% TAMADD
    %%%% This is particularly for sift features since my experience show that
    %%%% they cannot be tracked with descriptor more than 15 steps, I would
    %%%% delete features 15 steps after initialization
    if (step-features_info(i).init_frame)>20
        deletion_list = [deletion_list; i];
        continue
    end
    if numel(features_info) > 20
        if (step - features_info(i).last_visible)>20 %%% if a feature is not tracked for more than N steps most likely we have lost it
            deletion_list = [deletion_list; i];
        end
    end
    %     %%%
end
StatData.DELETION_LIST = deletion_list;
