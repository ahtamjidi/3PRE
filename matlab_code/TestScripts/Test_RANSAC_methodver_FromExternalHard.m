...
%     for scanIndex=1186:1192
    ...
    FileToSave = ['DataCam',num2str(idxCam),'_',num2str(idxRange(1)),'_',num2str(idxRange(1))];
    save(FileToSave) 
    save('LoadFileName','FileToSave')
end
clear all
close all
clc
load GroundTruth.mat
load('LoadFileName','FileToSave');
...
err_Euler2(:,scanIndex-idxRange(1)+1)=R2e(R_gt*inv(Rl_gicp));
euler_gt(:,scanIndex-idxRange(1)+1) = R2e(R_gt);
euler_RANSAC(:,scanIndex-idxRange(1)+1) = R2e(Rl_gicp);
vecTl(:,scanIndex-idxRange(1)+1) = Tl_gicp;
vecT_gt(:,scanIndex-idxRange(1)+1) = T_gt;

% Tl_gicp
disp('----------------------------------')
disp(NormError(scanIndex-idxRange(1)+1))
disp(180*err_Euler(:,scanIndex-idxRange(1)+1)/pi)
%     Data(scanIndex-1).Depth_Matched(2,:)
    %%
end
save('FullDataExternalHard','err_T','NormError','err_Euler2','err_Euler','euler_gt','euler_RANSAC','vecTl','vecT_gt')
