function [R,T,error,BestFit,support]=ICP_RANSAC3(Ya,Yb,preFeatures,currentFeatures,options)
...
%     [RR2, TT2] = icp(Ya,Yb);
    ...
    M(iter).SupportSet.Fa=[];
    M(iter).SupportSet.Yb=[];
    M(iter).SupportSet.Fb=[];
    for (i=1:size(Ya,2))
        try
        ...
        if TempError<=options.DistanceThreshold
            ...
            M(iter).SupportSet.Fa=[M(iter).SupportSet.Fa;preFeatures(i,:)];
            M(iter).SupportSet.Fb=[M(iter).SupportSet.Fb;currentFeatures(i,:)];
...
support = M(BestFitIdx).SupportSet;
